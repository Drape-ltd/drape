...
export function isFreshProviderAccount(input: { createdAt: unknown; lastSignInAt: unknown }) {
...
export function shouldChooseRoleAfterFreshProviderSignIn(input: {
  intentMode: unknown
  createdAt: unknown
  lastSignInAt: unknown
}) {
  return input.intentMode === 'sign-in' && isFreshProviderAccount(input)
}
...
export function shouldApplyFreshSignupRole(input: {
  ...
  return isFreshProviderAccount(input)
}
