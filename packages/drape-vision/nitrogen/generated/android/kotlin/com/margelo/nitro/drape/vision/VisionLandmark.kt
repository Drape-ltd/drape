///
/// VisionLandmark.kt
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

package com.margelo.nitro.drape.vision

import androidx.annotation.Keep
import com.facebook.proguard.annotations.DoNotStrip
import java.util.Objects


/**
 * Represents the JavaScript object/struct "VisionLandmark".
 */
@DoNotStrip
@Keep
data class VisionLandmark(
  @DoNotStrip
  @Keep
  val x: Double,
  @DoNotStrip
  @Keep
  val y: Double,
  @DoNotStrip
  @Keep
  val z: Double,
  @DoNotStrip
  @Keep
  val visibility: Double?,
  @DoNotStrip
  @Keep
  val presence: Double?
) {
  /* primary constructor */

  override fun equals(other: Any?): Boolean {
    if (this === other) return true
    if (other !is VisionLandmark) return false
    return Objects.deepEquals(this.x, other.x)
      && Objects.deepEquals(this.y, other.y)
      && Objects.deepEquals(this.z, other.z)
      && Objects.deepEquals(this.visibility, other.visibility)
      && Objects.deepEquals(this.presence, other.presence)
  }

  override fun hashCode(): Int {
    return arrayOf(
      x,
      y,
      z,
      visibility,
      presence
    ).contentDeepHashCode()
  }

  companion object {
    /**
     * Constructor called from C++
     */
    @DoNotStrip
    @Keep
    @Suppress("unused")
    @JvmStatic
    private fun fromCpp(x: Double, y: Double, z: Double, visibility: Double?, presence: Double?): VisionLandmark {
      return VisionLandmark(x, y, z, visibility, presence)
    }
  }
}
