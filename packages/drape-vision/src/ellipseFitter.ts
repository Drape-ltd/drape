...
export type EllipseWidthSample = {
  angleDegrees: number
  width: number
  weight?: number
}
...
export type EllipseFit = {
  semiMajor: number
  semiMinor: number
  axisAt0Degrees: number
  axisAt90Degrees: number
  circumference: number
  rmsError: number
  sampleCount: number
}

export function ramanujanCircumference(semiMajor: number, semiMinor: number) {
...
export function projectedEllipseWidth(semiMajor: number, semiMinor: number, angleDegrees: number) {
  if (semiMajor <= 0 || semiMinor <= 0) {
    throw new Error('Ellipse axes must be positive.')
  }
...
export function fitEllipseFromWidths(samples: EllipseWidthSample[]): EllipseFit {
  ...
  for (const sample of validSamples) {
    const theta = degreesToRadians(sample.angleDegrees)
