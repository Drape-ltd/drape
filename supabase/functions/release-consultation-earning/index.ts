...
Deno.serve(async (request) => {
  ...
  try {
    ...
    await Promise.allSettled([
      enqueuePushJob(supabase, { userId: booking.tailor_id, orderId: booking.order_id, source: FN, idempotencyKey: `${FN}:push:${booking.id}`, priority: 9, notification: { title: 'Consultation fee released', body: provider === 'STRIPE' ? 'Your consultation earning is in your Stripe balance. Bank arrival will be tracked separately.' : 'Your verified consultation earning was sent to your payout account.', preferenceKey: 'paymentReleased', data: notificationDestinationData({ kind: 'ORDER', orderId: booking.order_id }), communication: { category: 'PAYOUT', purpose: 'TRANSACTIONAL', severity: 'NOTICE', mandatory: true, inApp: true, destinationKey: 'ORDER_DETAIL', destinationParams: { orderId: booking.order_id }, deduplicationKey: `${FN}:${booking.id}:in-app` } } }),
