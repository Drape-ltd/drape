...
export function getMoneyApproverRecipients() {
  const configured = parseEmailList(Deno.env.get('OPS_MONEY_APPROVER_EMAILS'))
...
type CriticalOpsIssueNotificationInput = {
  issueNumber: number
  issueType: string
  severity: string
  title: string
...
export async function sendCriticalOpsIssueNotification(input: CriticalOpsIssueNotificationInput) {
  ...
  const response = await fetch(RESEND_API, {
    ...
    headers: {
      ...
      'User-Agent': 'drape-ops-notifications/1.0',
    },
    body: JSON.stringify({
...
type MoneyApprovalRequiredNotificationInput = {
  requestId: string
  reference: string
  actionLabel: string
  riskLevel: string
...
export async function sendMoneyApprovalRequiredNotification(input: MoneyApprovalRequiredNotificationInput) {
  const apiKey = Deno.env.get('RESEND_API_KEY')?.trim()
  if (!apiKey) return { ok: false as const, skipped: true as const }
  const recipients = getMoneyApproverRecipients()
  if (!recipients.length) return { ok: false as const, skipped: true as const }
  const opsBaseUrl = (Deno.env.get('OPS_WEB_BASE_URL') ?? 'https://ops.drapeon.co').replace(/\/+$/u, '')
  ...
  const response = await fetch(RESEND_API, {
    ...
    body: JSON.stringify({
      from: getOpsNotificationFrom(),
      to: recipients,
      subject: `Founder approval needed · ${input.reference}`,
