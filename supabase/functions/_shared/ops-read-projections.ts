...
async function reliability(supabase: SupabaseClient, environment: string) {
  const [incidents, providers, jobs, monitors, jobItems] = await Promise.all([
  ...
  assertResults([incidents, providers, jobs, monitors, jobItems], 'Reliability')
  ...
  return {
    ...
    jobs: { pending: numberValue(statusCounts.PENDING), retryable: numberValue(job.retryableCount), processing: numberValue(statusCounts.PROCESSING), dead: numberValue(job.deadCount), oldestPendingAt: text(job.oldestPendingAt), oldestProcessingAt: text(job.oldestProcessingAt) },
    jobItems: (jobItems.data ?? []).map((row) => ({ id: String(row.id), type: String(row.job_type), status: String(row.status), attempts: numberValue(row.attempt_count), maxAttempts: numberValue(row.max_attempts), runAt: String(row.run_at), createdAt: String(row.created_at), updatedAt: String(row.updated_at) })),
    observedAt: new Date().toISOString(),
  }
}
...
async function tailorDetail(supabase: SupabaseClient, input: Record<string, unknown>) {
  ...
  return {
    tailor: {
      id: String(profile.data.id), userId, name: text(profile.data.business_name) ?? text(profile.data.display_name) ?? 'Tailor', email: text(user.data?.email), bio: text(profile.data.bio), location: text(profile.data.location), region: text(user.data?.region_code), languages: Array.isArray(profile.data.languages) ? profile.data.languages.map(String) : [], specialties: Array.isArray(profile.data.specialty_tags) ? profile.data.specialty_tags.map(String) : [], priceMin: typeof profile.data.price_range_min === 'number' ? profile.data.price_range_min : null, priceMax: typeof profile.data.price_range_max === 'number' ? profile.data.price_range_max : null, currency: text(profile.data.currency), trustStatus: String(profile.data.id_verification_status ?? 'NOT_SUBMITTED'), payoutReady: Boolean(profile.data.payout_account_verified) && !Boolean(profile.data.payout_reverification_required), payoutProvider: text(profile.data.payout_provider), payoutCurrency: text(profile.data.payout_currency), live: Boolean(profile.data.is_live), profileComplete: Boolean(profile.data.profile_completed), availability: text(profile.data.availability), shopPaused: Boolean(profile.data.shop_paused), totalOrders: Number(profile.data.total_orders ?? 0), rating: typeof profile.data.avg_rating === 'number' ? profile.data.avg_rating : null, createdAt: String(profile.data.created_at), updatedAt: String(profile.data.updated_at),
    },
