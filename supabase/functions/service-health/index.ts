...
function envCheck(name: string, required = true): Check {
  ...
  return {
    ...
    message: required
      ...
      : "Optional environment variable not configured",
  };
}
...
function secretModeCheck(
  names: string[],
  label: string,
  modes: Record<string, "test" | "live">,
): Check {
  const value = names
  ...
  return {
    ...
    status: "warn",
    message:
      `${label} is configured, but the key prefix did not match a known test/live pattern.`,
...
function smsSecretCheck(): Check {
  ...
  const termiiConfigured = !!(Deno.env.get("TERMII_API_KEY") &&
    (Deno.env.get("TERMII_SENDER_ID") ?? Deno.env.get("TERMII_FROM")));
  const twilioConfigured = !!(Deno.env.get("TWILIO_ACCOUNT_SID") &&
    Deno.env.get("TWILIO_AUTH_TOKEN") && Deno.env.get("TWILIO_FROM_NUMBER"));
  ...
  if (provider === "termii") {
    return termiiConfigured
      ...
      : {
        ...
        status: "fail",
        message:
          "SMS_PROVIDER=termii but TERMII_API_KEY or TERMII_SENDER_ID is missing",
      };
  }
  ...
  if (provider === "twilio") {
    return twilioConfigured
      ...
      : {
        ok: false,
        status: "fail",
        message: "SMS_PROVIDER=twilio but Twilio credentials are missing",
      };
  }

  if (termiiConfigured) {
    return { ok: true, status: "ok", message: "Termii SMS configured" };
  }
  if (twilioConfigured) {
    return { ok: true, status: "ok", message: "Twilio SMS configured" };
  }
  ...
  return {
    ...
    status: "warn",
    message:
      "SMS provider is not configured. Push and email fallback can still run.",
  };
}

function taxPolicyReviewCheck(
  ...
  const blocked = TAX_POLICY_CONTROLS.filter((policy) =>
    policy.mode === "BLOCKED"
  );
  ...
  if (overdue.length > 0) {
    return {
      ...
      message:
        `${overdue.length} tax policy review(s) are overdue. Public checkout must remain blocked until official sources are reviewed.`,
      details: {
  ...
  return {
    ...
    message: blocked.length > 0
      ? `Tax policies are current; ${
        blocked.map((policy) => policy.countryCode).join(", ")
      } checkout remains intentionally blocked pending country-specific tax support.`
...
async function activatedTaxControlHealthCheck(
  supabase: any,
  readinessTier: ReadinessTier,
): Promise<Check> {
  const { data, error } = await supabase.from("tax_control_health")
    .select(
      "activation_id,environment,policy_version,jurisdiction_country_code,origin_country_code,destination_country_code,tax_transaction_type,fulfillment_classification,review_due_at,health_status,affected_open_reservations,correlation_id",
    );
  if (error) {
    return {
      ok: false,
      status: "fail",
      message: `Could not read activated tax-control health: ${error.message}`,
    };
  }
  const rows = data ?? [];
  const expired = rows.filter((row: any) => row.health_status === "EXPIRED");
  const due = rows.filter((row: any) => row.health_status === "REVIEW_DUE");
  const productionExpired = expired.filter((row: any) =>
  ...
  return {
    ...
    details: {
      ...
      expired: expired.map((row: any) => ({
        activation_id: row.activation_id,
        environment: row.environment,
        correlation_id: row.correlation_id,
      })),
      review_due: due.map((row: any) => ({
        activation_id: row.activation_id,
        environment: row.environment,
...
async function reviewedTaxDependencyHealthCheck(
  supabase: any,
  readinessTier: ReadinessTier,
): Promise<Check> {
  const sources = [
    ...
    {
      table: "tax_registration_controls",
      id: "id",
      type: "REGISTRATION",
      hasStatus: true,
    },
    {
      table: "tax_responsibility_controls",
      id: "id",
      type: "RESPONSIBILITY",
      hasStatus: true,
    },
    {
      table: "tax_corridor_controls",
      id: "id",
      type: "CORRIDOR",
      hasStatus: true,
    },
    {
      table: "tax_line_classification_controls",
      id: "id",
      type: "LINE_CLASSIFICATION",
      hasStatus: false,
    },
    {
      table: "tax_registration_facts",
      id: "id",
      type: "REGISTRATION_FACT",
      hasStatus: false,
    },
  ] as const;
  const now = Date.now();
  ...
  for (const source of sources) {
    ...
    if (error) {
      return {
        ...
        status: "fail",
        message:
          `Could not read ${source.type.toLowerCase()} review health: ${error.message}`,
      };
    }
    for (const row of data ?? []) {
      if (source.hasStatus && !["APPROVED", "ACTIVE"].includes(row.status)) {
        continue;
      }
...
async function databaseCheck(supabase: any): Promise<Check> {
  ...
  return {
    ...
    message: latencyMs > 1_500
      ? "Database reachable but slower than expected"
      : "Database reachable",
    latencyMs,
  };
}
...
async function cronCheck(supabase: any): Promise<Check> {
  ...
  if (error) {
    return {
      ...
      status: "warn",
      message:
        "Cron status RPC is not installed yet; liveness and database checks still passed.",
      latencyMs,
    };
  }
  ...
  const vaultServiceRoleConfigured =
    payload.vaultServiceRoleConfigured === true;
  ...
  if (
    !vaultAvailable || !vaultProjectUrlConfigured || !vaultServiceRoleConfigured
  ) {
    return {
      ok: false,
      status: "fail",
      message:
        "Cron cannot invoke Edge Functions until DB Vault has project_url and service_role_key configured",
      latencyMs,
    };
  }
...
async function jobQueueCheck(supabase: any): Promise<Check> {
  ...
  const payload = data && typeof data === "object"
    ? data as Record<string, unknown>
    : {};
  const deadCount = typeof payload.deadCount === "number"
    ? payload.deadCount
    : 0;
  const retryableCount = typeof payload.retryableCount === "number"
    ? payload.retryableCount
    : 0;
  const oldestPendingAt = typeof payload.oldestPendingAt === "string"
    ? payload.oldestPendingAt
    : null;
  ...
  if (deadCount > 0) {
    return {
      ...
      status: "fail",
      message:
        `${deadCount} background job(s) are dead-lettered and need ops review`,
      latencyMs,
    };
  }
...
async function slackDeliveryCheck(supabase: any): Promise<Check> {
  ...
  const { data, error } = await supabase
    ...
    .select("status,channel_key,error_code,updated_at")
    .gte("updated_at", since)
    .in("status", ["RETRYABLE", "DEAD"])
    .limit(100);
  const latencyMs = Math.round(performance.now() - startedAt);
  if (error) {
    return {
      ok: false,
      status: "fail",
      message: `Slack delivery ledger is unavailable: ${error.message}`,
      latencyMs,
    };
  }
  const rows = Array.isArray(data) ? data : [];
  const dead = rows.filter((row: any) => row.status === "DEAD");
  const retryable = rows.filter((row: any) => row.status === "RETRYABLE");
  if (dead.length > 0) {
    return {
      ok: false,
      status: "fail",
      message:
        `${dead.length} Slack Ops delivery attempt(s) reached a dead terminal outcome in the last 24 hours.`,
      latencyMs,
      details: { dead: dead.length, retryable: retryable.length },
    };
  }
  return {
    ok: true,
    status: retryable.length > 0 ? "warn" : "ok",
    message: retryable.length > 0
      ? `${retryable.length} Slack Ops delivery attempt(s) are retrying.`
      : "Slack Ops deliveries have no unresolved failures in the last 24 hours.",
    latencyMs,
    details: { dead: 0, retryable: retryable.length },
  };
}
  ...
  if (open.length > 0) {
    return {
      ...
      message: `Provider circuit open: ${
        open.map((row) => `${row.provider}:${row.operation}`).join(", ")
      }`,
      latencyMs,
    };
  }
  ...
  if (degraded.length > 0) {
    return {
      ...
      message: `Provider degraded: ${
        degraded.map((row) => `${row.provider}:${row.operation}`).join(", ")
      }`,
      latencyMs,
    };
  }
  ...
  try {
    ...
    if (overdue.length > 0) {
      ...
      return {
        ...
        status: "fail",
        message:
          `${overdue.length} payout(s) are overdue without a payout row. Oldest is ${oldest.minutesPastReady} minutes past ready.`,
        latencyMs,
        details: {
...
async function androidPushRegistrationCheck(supabase: any): Promise<Check> {
  ...
  if (error) {
    return {
      ...
      status: "warn",
      message:
        `Could not inspect Android push token registration: ${error.message}`,
      latencyMs,
    };
  }
  ...
  if (!updatedAt) {
    return {
      ...
      status: "warn",
      message:
        "No Android push token has registered yet. Confirm Firebase google-services.json, EAS FCM credentials, rebuild, then open Android QA devices.",
      latencyMs,
    };
  }
  ...
  if (!Number.isFinite(ageMs) || ageMs > 7 * 24 * 60 * 60 * 1000) {
    return {
      ...
      status: "warn",
      message:
        `Latest Android push token is stale (${updatedAt}). Reopen a rebuilt Android app and confirm expo-notifications stores a fresh token.`,
      latencyMs,
    };
  }
...
async function pushReceiptCheck(supabase: any): Promise<Check> {
  ...
  const pending = rows.filter((row) =>
    row.status === "TICKET_ACCEPTED" || row.status === "RECEIPT_PENDING"
  );
  const oldestPendingAt = pending
    .map((row) =>
      typeof row.ticket_created_at === "string" ? row.ticket_created_at : null
    )
  ...
  const oldestPendingAgeMs = oldestPendingAt
    ? Date.now() - new Date(oldestPendingAt).getTime()
    : 0;
  const credentialErrors = deliveryErrors.filter((row) =>
    row.error_code === "InvalidCredentials"
  );
  ...
  if (credentialErrors.length > 0) {
    return {
      ...
      status: "fail",
      message:
        `${credentialErrors.length} push receipt(s) report invalid APNs/FCM credentials`,
      latencyMs,
      details: {
        delivery_errors: deliveryErrors.length,
  ...
  if (
    deliveryErrors.length > 0 || expired.length > 0 ||
    ...
    return {
      ...
      status: "warn",
      message:
        `${deliveryErrors.length} delivery error(s), ${expired.length} expired receipt(s), and ${pending.length} pending receipt(s) in the last 24 hours`,
      latencyMs,
      details: {
        delivery_errors: deliveryErrors.length,
        expired: expired.length,
...
function providerSecretChecks(readinessTier: ReadinessTier) {
  return {
    ...
    stripeWebhookSecret: anyEnvCheck([
      "STRIPE_WEBHOOK_SECRET",
      "STRIPE_WEBHOOK_SECRETS",
    ], "STRIPE_WEBHOOK_SECRET"),
    paystackSecret: anyEnvCheck([
      "PAYSTACK_SECRET_KEY",
    ...
    ziptaxSecret: Deno.env.get("ZIPTAX_API_KEY")
      ...
      ? {
        ...
        status: "warn",
        message:
          "ZIPTAX_API_KEY is not configured. Beta checkout can use the explicit static tax fallback, but public US/Canada checkout is not launch-ready.",
      }
      : envCheck("ZIPTAX_API_KEY"),
    authSmsHookSecret: anyEnvCheck(
      ["AUTH_SMS_HOOK_SECRET", "SUPABASE_AUTH_HOOK_SECRET"],
      "AUTH_SMS_HOOK_SECRET",
      false,
    ),
    reauthProofSecret: anyEnvCheck([
      "REAUTH_PROOF_SECRET",
      "DRAPE_REAUTH_PROOF_SECRET",
    ], "REAUTH_PROOF_SECRET"),
    healthcheckSecret: anyEnvCheck([
      "DRAPE_HEALTHCHECK_SECRET",
      "HEALTHCHECK_SECRET",
    ], "DRAPE_HEALTHCHECK_SECRET"),
    sentryDsn: anyEnvCheck(
      ["SENTRY_DSN", "SUPABASE_SENTRY_DSN"],
      "SENTRY_DSN",
      false,
    ),
    sentryOpsWebhookSecret: envCheck("SENTRY_OPS_WEBHOOK_SECRET"),
    sentryMonitorToken: envCheck("SENTRY_MONITOR_TOKEN"),
    sentryMonitorOrg: envCheck("SENTRY_MONITOR_ORG"),
    slackBotToken: envCheck("SLACK_BOT_TOKEN"),
    slackEngineeringChannel: envCheck("SLACK_CHANNEL_ENGINEERING_ERRORS"),
    slackCriticalChannel: envCheck("SLACK_CHANNEL_OPS_CRITICAL"),
    slackDeliveryChannel: envCheck("SLACK_CHANNEL_OPS_DELIVERY"),
    slackIntakeChannel: envCheck("SLACK_CHANNEL_OPS_INTAKE"),
    slackMoneyChannel: envCheck("SLACK_CHANNEL_OPS_MONEY"),
    slackSafetyChannel: envCheck("SLACK_CHANNEL_OPS_SAFETY"),
...
Deno.serve(async (req) => {
    ...
    return jsonResponse(
      {
        ...
        message:
          "This readiness check requires the Drapeon healthcheck secret.",
      },
      401,
      cors,
    );
  ...
  const checks: Record<string, Check> = {
    ...
    serviceRoleKey: {
      ok: !!(Deno.env.get("DRAPE_SERVICE_ROLE_JWT") ??
        Deno.env.get("DRAPE_SERVICE_ROLE_KEY") ??
        Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")),
      status: Deno.env.get("DRAPE_SERVICE_ROLE_JWT") ??
          Deno.env.get("DRAPE_SERVICE_ROLE_KEY") ??
          Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")
        ? "ok"
        : "fail",
      message: Deno.env.get("DRAPE_SERVICE_ROLE_JWT") ??
          Deno.env.get("DRAPE_SERVICE_ROLE_KEY") ??
          Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")
        ? "Configured"
    ...
    activatedTaxControls: await activatedTaxControlHealthCheck(
      supabase,
      readinessTier,
    ),
    reviewedTaxDependencies: await reviewedTaxDependencyHealthCheck(
      supabase,
      readinessTier,
    ),
  ...
  const warnings = Object.values(checks).filter((item) =>
    item.status === "warn"
  );
