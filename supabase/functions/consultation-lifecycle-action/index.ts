...
Deno.serve(async (request) => {
  ...
  await Promise.allSettled([
    enqueuePushJob(supabase, { userId: counterpartId, orderId: booking.order_id, source: FN, idempotencyKey: `${FN}:push:${booking.id}:${parsed.data.idempotencyKey}`, priority: 10, notification: { title: 'Consultation cancelled', body, preferenceKey: 'orderUpdates', data: notificationDestinationData({ kind: 'ORDER', orderId: booking.order_id }), communication: { category: 'MESSAGE', purpose: 'TRANSACTIONAL', severity: 'NOTICE', inApp: true, destinationKey: 'ORDER_DETAIL', destinationParams: { orderId: booking.order_id }, deduplicationKey: `${FN}:${booking.id}:${parsed.data.idempotencyKey}:in-app` } } }),
