-- Remote schema has historical UUID/text drift on a few hot RPC paths.
-- Normalize comparisons to text on both sides so the functions lint and run
-- regardless of whether the underlying column is uuid or legacy text.
