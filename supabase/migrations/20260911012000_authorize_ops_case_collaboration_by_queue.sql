-- Bind generic case collaboration to the active queue policy and add a
-- durable escalation transition. This is a forward repair to the already
-- deployed collaboration function; the prior migration remains immutable.
...
begin
  if v_action not in ('ACKNOWLEDGE', 'ASSIGN_SELF', 'ADD_NOTE', 'ESCALATE') then
  ...
  if v_action = 'ESCALATE' and (length(v_reason) < 12 or length(v_reason) > 1000) then
    raise exception 'An escalation reason between 12 and 1000 characters is required.' using errcode = '22023';
  end if;
  ...
  where policy.environment = v_environment
    and policy.queue_key = v_issue.queue_key
    and policy.active
  ...
  then
    raise exception 'Workforce principal does not own this case queue.' using errcode = '42501';
  end if;
  if v_action = 'ESCALATE' and not ('escalate' = any(v_policy.permitted_actions)) then
    raise exception 'Escalation is not permitted by this queue policy.' using errcode = '42501';
  end if;
  if v_action = 'ASSIGN_SELF' and not ('assign' = any(v_policy.permitted_actions)) then
    raise exception 'Assignment is not permitted by this queue policy.' using errcode = '42501';
  end if;
  if v_action = 'ACKNOWLEDGE'
    and not ('acknowledge' = any(v_policy.permitted_actions) or 'triage' = any(v_policy.permitted_actions))
  then
    raise exception 'Acknowledgement is not permitted by this queue policy.' using errcode = '42501';
  end if;
  ...
  v_next_status := case
    when v_action = 'ESCALATE' then 'ESCALATED'
  ...
  v_event_type := case
    when v_action = 'ADD_NOTE' then 'INTERNAL_NOTE'
    when v_action = 'ASSIGN_SELF' then 'ASSIGNMENT'
  ...
  v_summary := case
    ...
    when v_action = 'ESCALATE' then 'A named operator escalated the case under its queue policy.'
    else v_reason
  end;
  ...
  insert into public.ops_action_receipts (
    ...
    case
      ...
      when v_action = 'ADD_NOTE' then 'Internal case note persisted.'
      else 'Case escalation and backup ownership persisted.'
    end,
    p_correlation_id,
    '[]'::jsonb,
    '[]'::jsonb,
    case
      when v_action = 'ACKNOWLEDGE' then v_issue.recommended_action
      when v_action = 'ESCALATE' then 'Backup team ' || v_policy.backup_team || ' must review this escalation.'
      else null
    end,
    v_now
  ...
  set canonical_status = v_next_status,
      status = case when v_next_status = 'NEW' then 'OPEN' when v_next_status = 'ESCALATED' then 'ESCALATED' else 'IN_REVIEW' end,
      ...
      first_responded_at = case when v_action in ('ACKNOWLEDGE', 'ESCALATE') then coalesce(first_responded_at, v_now) else first_responded_at end,
      metadata = case when v_action = 'ESCALATE' then coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
        'lastEscalatedAt', v_now,
        'lastEscalatedBy', trim(p_actor_label),
        'lastEscalationReason', v_reason,
        'escalatedToTeam', v_policy.backup_team
      ) else metadata end
  where id = p_issue_id
  ...
  insert into public.ops_case_events (
    ...
    jsonb_build_object(
      ...
      'queuePolicyVersion', v_policy.version,
      'backupTeam', case when v_action = 'ESCALATE' then v_policy.backup_team else null end
    ),
  ...
  insert into public.ops_audit_logs (
    ...
    nullif(v_reason, ''),
    jsonb_build_object('recordVersion', p_expected_record_version, 'caseStatus', v_previous_status, 'queuePolicyVersion', v_policy.version),
    jsonb_build_object('caseStatus', v_issue.canonical_status, 'recordVersion', v_issue.record_version, 'backupTeam', case when v_action = 'ESCALATE' then v_policy.backup_team else null end),
    v_now
  );
...
comment on function public.perform_ops_case_collaboration_action(uuid,text,text,bigint,text,uuid,text,text,uuid) is
  'Idempotent, environment-bound acknowledgement, queue-authorized ownership, internal-note, and escalation boundary for named Ops workforce principals.';
