-- Keep the staged-settlement initializer compatible with the legacy orders
-- table, whose party identifiers are stored as text while the settlement
-- contract deliberately uses auth.users UUIDs.
...
revoke all on function public.initialize_order_settlement_plan(text) from public,anon,authenticated;
grant execute on function public.initialize_order_settlement_plan(text) to service_role;
