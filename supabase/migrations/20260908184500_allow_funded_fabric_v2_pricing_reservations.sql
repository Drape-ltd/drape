-- Funded-fabric v2 orders use the same protected checkout reservation contract
-- as v1. Preserve the exact accepted-quote comparisons while allowing the
-- active v2 policy introduced on 2026-08-21.
...
begin
  ...
  if p_fabric_funding_policy_version not in (
      'fabric-funding-2026-08-01-v1',
      'fabric-funding-2026-08-21-v2'
    )
...
comment on function public.create_funded_commercial_pricing_reservation(text,uuid,text,uuid,text,currency,integer,integer,integer,integer,integer,text,text,boolean,jsonb,uuid,text,text,integer,integer,jsonb,text,timestamptz) is
  'Creates an idempotent protected checkout pricing reservation for accepted funded-fabric v1 and v2 quotes.';
