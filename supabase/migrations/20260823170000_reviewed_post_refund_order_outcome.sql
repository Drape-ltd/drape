-- A successful partial refund may initially remain under review. Ops can later
-- make one audited, non-monetary order decision without rewriting the approved
-- refund or silently moving the remaining protected balance.

alter table public.order_refund_resolutions
  add column if not exists reviewed_order_outcome text,
  add column if not exists reviewed_resume_stage public.order_stage,
  add column if not exists reviewed_outcome_reason text,
  add column if not exists reviewed_outcome_actor_email text,
  add column if not exists reviewed_outcome_applied_at timestamptz;

alter table public.order_refund_resolutions
  drop constraint if exists order_refund_resolutions_reviewed_order_outcome_check;
alter table public.order_refund_resolutions
  add constraint order_refund_resolutions_reviewed_order_outcome_check
  check (reviewed_order_outcome is null or reviewed_order_outcome in ('CONTINUE_ORDER','CLOSE_ORDER'));
...
begin
  ...
  if p_order_outcome='CONTINUE_ORDER' then
    ...
    if v_order.stage::text='IN_DISPUTE' then
      ...
      values(v_order.id,v_resume,'Production resumed after the completed partial refund and Ops review.');
    end if;
    ...
    if v_order.stage::text='IN_DISPUTE' then
      perform public.finalize_order_terminal(
        v_order.id,'COMPLETE',null,'OPS','ops.partial_refund_order_closed_after_review',
        'Order closed after a completed partial refund and reviewed terminal decision.',
