-- Durable preflight receipt for a protected tailor trust decision. The existing
-- verification worker remains the domain transition owner during parallel run;
-- this preflight prevents anonymous, stale, cross-environment, or untracked
-- decisions from reaching it.
...
begin
  ...
  if v_issue.id is null
    ...
    or v_issue.tailor_profile_id is distinct from p_profile_id
    or coalesce(v_issue.user_id, v_issue.actor_id) is distinct from p_tailor_user_id
