-- A crashed worker used to leave jobs in PROCESSING. The next recovery claim
-- reclaimed them even after max_attempts, producing an unbounded retry loop.
...
begin
  ...
  with exhausted as (
    ...
    select
      ...
      coalesce(locked_by, p_worker_id),
      'DEAD',
  ...
  select
    ...
    coalesce(
      nullif(exhausted.payload ->> 'recipientUserId', '')::uuid,
      nullif(exhausted.payload ->> 'userId', '')::uuid
    ),
    coalesce(
