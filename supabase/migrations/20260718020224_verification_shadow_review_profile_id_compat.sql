-- Keep verification review decisions portable across projects where
-- tailor_profiles.id is either text or uuid while request records store it as text.
...
begin
  ...
  select tailor_profiles.id into v_tailor_profile_id
  from public.tailor_profiles
  where tailor_profiles.id::text = req.tailor_profile_id
  for update;
  ...
  if upper(trim(p_decision)) = 'APPROVE' then
    ...
    set
      ...
      updated_at = now()
    where tailor_profiles.id = v_tailor_profile_id;
