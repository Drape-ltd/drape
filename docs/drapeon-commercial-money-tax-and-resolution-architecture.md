...
Payment-webhook and tip-payout jobs wake the dedicated money worker when their durable queue row commits, so notification volume cannot delay money state and an empty queue creates no `pg_net` response traffic. A conditional five-minute recovery sweep handles delayed retries, stale claims, or a missed wakeup without invoking Edge when no money job is due. Service health requires that recovery schedule and the shared queue health check fails when any webhook processing or reconciliation job dead-letters. Production promotion requires replay of one signed event per provider, duplicate delivery, a forced retry, a forced dead letter, provider reconciliation, exact database and ledger outcomes, and an Ops/Sentry correlation review.
