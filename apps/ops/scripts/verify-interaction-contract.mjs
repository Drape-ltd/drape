...
const checks = [
  ...
  ['Local dry-run sign-in distinguishes the reviewer from the founder approver without accepting arbitrary identities', opsLayout.includes('Continue as Ops reviewer') && opsLayout.includes('Continue as founder approver') && localUnlockRoute.includes("candidate.key === identityKey") && localUnlockRoute.includes("error: 'invalid-local-identity'") && opsAuth.includes("createHmac('sha256', token)") && opsAuth.includes("OPS_MONEY_APPROVER_EMAILS ?? 'founders@drapeon.co'")],
