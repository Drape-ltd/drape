...
const opsLayout = await readFile(new URL('app/ops/layout.tsx', root), 'utf8')
const localUnlockRoute = await readFile(new URL('app/ops/local-unlock/route.ts', root), 'utf8')
const opsConsole = await readFile(new URL('../web/lib/ops-console.ts', root), 'utf8')
const moneyDesk = await readFile(new URL('../web/lib/money-desk.ts', root), 'utf8')
const moneyPage = await readFile(new URL('app/ops/money/page.tsx', root), 'utf8')
const moneyActionRoute = await readFile(new URL('app/ops/api/actions/money/route.ts', root), 'utf8')
...
const trustActionRoute = await readFile(new URL('app/api/actions/trust/route.ts', root), 'utf8')
const trustActionEdge = await readFile(new URL('../../supabase/functions/ops-trust-action/index.ts', root), 'utf8')
...
const deadJobResolutionMigration = await readFile(new URL('../../supabase/migrations/20260913151000_audited_dead_job_resolution.sql', root), 'utf8')
...
const checks = [
  ...
  ['Local dry-run sessions are bound to one server-configured identity and cannot self-select founder authority', opsLayout.includes('Continue as configured workforce identity') && !opsLayout.includes('Continue as founder approver') && !localUnlockRoute.includes('formData') && localUnlockRoute.includes('getLocalWorkforceDryRunIdentity()') && opsAuth.includes("createHmac('sha256', token)") && opsAuth.includes('local-workforce-v2:${identity.key}:${identity.email}:${identity.role}')],
  ['The founder principal can perform its required work but Money Desk authority remains exact-email, fresh-MFA, desktop, and scoped-grant bound', opsConsole.includes("admin: OPS_LIVE_SECTIONS.map((section) => section.key)") && ['money-desk-elevation', 'money-desk-request', 'money-desk-decision', 'money-desk-execution'].every((permission) => opsConsole.includes(`'${permission}'`)) && moneyPage.includes('isFounderMoneyDeskApprover(session.email)') && moneyActionRoute.includes("(action === 'DECIDE' || action === 'EXECUTE') && !isFounderMoneyDeskApprover(session.email)") && moneyActionRoute.includes('hasFreshOpsMfa(session)') && moneyActionRoute.includes('isRestrictedOpsPhoneHeaders') && moneyActionRoute.includes('getActiveMoneyDeskGrant') && moneyDesk.includes("DEFAULT_FOUNDER_MONEY_APPROVER = 'founders@drapeon.co'")],
  ...
  ['Trust decisions require both the server-only production broker, a fresh signed sensitive-app assertion, and a terminal case outcome', trustActionRoute.includes('getSupabaseServiceRoleKey()') && trustActionRoute.includes('validateServiceRoleTarget(') && trustActionRoute.includes('Authorization: `Bearer ${serviceRoleKey}`') && trustActionEdge.includes('hasVerifiedServiceRoleBroker(request)') && trustActionEdge.includes("claims.role === 'service_role'") && trustActionEdge.includes('claims.ref === expectedProjectRef') && trustActionEdge.includes('authorization !== apiKey') && /\[functions\.ops-trust-action\]\s*verify_jwt = true/u.test(supabaseConfig) && trustActionEdge.includes('sensitiveAudiences.some((audience) => identity.audiences.includes(audience))') && trustActionEdge.includes('Math.floor(Date.now() / 1000) - identity.issuedAt <= 15 * 60') && trustActionEdge.includes("{ type: 'OPS_CASE', status: caseResolved ? 'RESOLVED' : 'FAILED' }")],
  ...
  ['Dead-job resolution is Reliability-only, reason-bound, named, and never replays terminal work', casePanel.includes("act('RESOLVE_DEAD_JOB')") && casePanel.includes('Resolve without replay') && caseRoute.includes("action === 'RESOLVE_DEAD_JOB'") && caseEdge.includes("action === 'RESOLVE_DEAD_JOB'") && deadJobResolutionMigration.includes("v_issue.queue_key <> 'reliability'") && deadJobResolutionMigration.includes("v_job.status <> 'DEAD'") && deadJobResolutionMigration.includes("'deadJobDisposition', 'ACKNOWLEDGED_NO_REPLAY'") && !deadJobResolutionMigration.includes("update public.job_queue\n  set")],
  ['Reviewed dead jobs remain terminal evidence while only explicit dispositions leave actionable health', deadJobResolutionMigration.includes("issue.canonical_status in ('RESOLVED', 'CLOSED')") && deadJobResolutionMigration.includes("'totalDeadCount'") && deadJobResolutionMigration.includes("'reviewedDeadCount'") && providersPage.includes('Reviewed terminal') && communicationsPage.includes('No stale message was replayed')],
