...
const checks = [
  ...
  ['Trust decisions require both the server-only production broker and a fresh signed sensitive-app assertion', trustActionRoute.includes('getSupabaseServiceRoleKey()') && trustActionRoute.includes('validateServiceRoleTarget(') && trustActionRoute.includes('Authorization: `Bearer ${serviceRoleKey}`') && trustActionEdge.includes('hasVerifiedServiceRoleBroker(request)') && trustActionEdge.includes("claims.role === 'service_role'") && trustActionEdge.includes('claims.ref === expectedProjectRef') && trustActionEdge.includes('authorization !== apiKey') && /\[functions\.ops-trust-action\]\s*verify_jwt = true/u.test(supabaseConfig) && trustActionEdge.includes('sensitiveAudiences.some((audience) => identity.audiences.includes(audience))') && trustActionEdge.includes('Math.floor(Date.now() / 1000) - identity.issuedAt <= 15 * 60')],
