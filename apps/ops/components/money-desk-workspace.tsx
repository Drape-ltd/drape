  ...
  return (
    <article className="ops-money-card" id={`money-${request.id}`}>
      ...
      <div className="ops-money-grid">
        ...
        <div><span>Independent approval</span><strong>{request.approvalCount} of {request.requiredApprovalCount}</strong><small>{request.decisions.length > 0 ? request.decisions.map((decision) => `${formatEnum(decision.decision)} · ${decision.approverRole}`).join(' · ') : 'No decision recorded yet'}</small></div>
  ...
  return (
    <>
      <section className="ops-summary-grid" aria-label="Money Desk summary">
        <Link className="ops-summary-cell ops-summary-link" href="/ops/money?view=approval#money-request-ledger"><div className="ops-summary-label">Awaiting approval</div><div className="ops-summary-value">{approvals}</div><div className="ops-summary-detail">Maker and checker stay independent</div></Link>
      ...
      <section className="ops-money-control" data-ready={sensitiveAccessReady}>
        ...
        <div><p className="ops-action-label">Protected money movement</p><h2>{sensitiveAccessReady ? 'Fresh MFA assurance is active' : 'Verify protected access before acting'}</h2><p>Preparation, independent approval, and provider execution remain separate durable states. Reading this queue never grants authority to move money.</p></div>
