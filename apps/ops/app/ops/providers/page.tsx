...
export default async function ProvidersPage({ searchParams }: { searchParams: Promise<{ providerState?: string; jobState?: string }> }) {
  ...
  const visibleJobs = selectedJobState ? data.jobItems.filter((job) => job.status.toUpperCase() === selectedJobState) : data.jobItems
  ...
  return (
    <>
      ...
      {queueRisk > 0 ? <div className="ops-status-banner ops-banner-after-grid" data-tone={data.jobs.dead > 0 ? 'critical' : 'warning'} role="alert"><AlertTriangle size={16} />The job queue has {data.jobs.dead} dead and {data.jobs.retryable} retryable job{queueRisk === 1 ? '' : 's'}. Inspect the exact source rows and linked reliability cases before replaying anything.</div> : null}
