...
export async function shareCustomerReferral(userId: string, displayName: string) {
  if (!ensureValue(userId, 'Your referral link is not ready yet. Please try again in a moment.')) return
...
export async function shareTailorProfile(tailorProfileId: string, tailorName: string) {
  if (!ensureValue(tailorProfileId, 'This profile is not ready to share yet. Please try again in a moment.')) return
...
export async function referToTailor(tailorProfileId: string, tailorName: string, referrerId: string) {
  if (!ensureValue(tailorProfileId, 'This tailor link is not ready yet. Please try again in a moment.')) return
  if (!ensureValue(referrerId, 'Your referral link is not ready yet. Please try again in a moment.')) return
...
export async function inviteTailorColleague(tailorId: string, tailorName: string) {
  if (!ensureValue(tailorId, 'Your invite link is not ready yet. Please try again in a moment.')) return
...
export async function sharePassportInvite(passportId: string, clientName: string, tailorName: string) {
  if (!ensureValue(passportId, 'This passport invite is not ready yet. Please try again in a moment.')) return
...
export async function inviteCustomerFromTailor(tailorProfileId: string, tailorName: string) {
  if (!ensureValue(tailorProfileId, 'Your profile is not ready to share yet. Please try again in a moment.')) return
  const link = `${BASE_URL}/tailor/${tailorProfileId}`
