...
export default function TailorDeleteAccountScreen() {
  ...
  const canSubmit =
    ...
    (providerReauth !== null || password.length > 0) &&
    !statusError
  const fallbackRoute = '/(tailor)/profile/account-settings' as const
  const returnTo =
    pickSafeReturnTo(params.historyChain, params.returnTo, fallbackRoute) ?? fallbackRoute
  ...
  async function handleSubmit() {
    ...
    if (!providerReauth && !password) {
      Alert.alert(
        'Password required',
        'Enter your current password before submitting this deletion request.'
      )
      return
    }
    if (!user?.email) {
      Alert.alert(
        'Session expired',
        'Please sign in again before submitting this deletion request.'
      )
      return
    }
    setSubmitting(true)
    ...
    if (providerReauth) {
      ...
      if (providerReauth === 'apple' && !appleAuthorizationCode) {
        setSubmitting(false)
        Alert.alert(
          'Apple confirmation incomplete',
          'Apple did not return the authorization needed to disconnect your account. Please try again.'
        )
        return
      }
    }
    ...
    if (proofResult.error || !proofResult.proof) {
      ...
      Alert.alert(
        ...
        proofResult.error ?? 'Confirm your identity again before continuing.'
      )
      return
    }
        ...
        <View style={styles.body}>
          <View style={styles.heroCard}>
            ...
            <Text style={styles.heroCopy}>
              Your request is recorded. We’ll send updates by email and in-app notification as it
              moves through review.{' '}
              {requestState.activeOrderCount > 0
                ? `We found ${requestState.activeOrderCount} active order${requestState.activeOrderCount === 1 ? '' : 's'}, so Drapeon will resolve open transactions before deletion or anonymization.`
                : 'We may contact you if we need confirmation.'}
            </Text>
          </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{requestState.status.replaceAll('_', ' ')}</Text>
            </View>
            <View style={styles.detailRow}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{requestState.activeOrderCount}</Text>
            </View>
          ...
          <View style={styles.noteCard}>
            ...
            <Text style={styles.noteCopy}>
              Email {CONTACTS.privacy} from your account email if anything about the request changes
              or you need to add more context.
            </Text>
          </View>
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        <View style={styles.body}>
          ...
          <TouchableOpacity
            ...
            accessibilityRole="button"
            accessibilityLabel="Retry deletion request status check"
          >
      ...
      >
        ...
        >
          <View style={styles.heroCard}>
            ...
            <Text style={styles.heroCopy}>
              This does not behave like an instant total wipe. Drapeon may retain limited records
              where required for security, fraud prevention, active transactions, legal obligations,
              or claims handling.
            </Text>
          </View>
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.sectionCopy}>
              Your account can be restricted and queued for deletion review. If you have active
              orders, payouts, disputes, or legal retention obligations, Drapeon resolves those
              before final deletion or anonymization.
            </Text>
            <View style={styles.divider} />
            <Text style={styles.sectionTitle}>Account email</Text>
            <Text style={styles.accountValue}>
              {user?.email ?? 'No account email available in-app'}
            </Text>
          </View>
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.sectionCopy}>
              Optional, but helpful for support and privacy handling.
            </Text>
            <TextInput
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.sectionCopy}>
              Type DELETE, then{' '}
              {providerReauth
                ? `continue with ${providerReauth === 'apple' ? 'Apple' : 'Google'}`
                : 'confirm your current password'}
              . This keeps an unlocked phone from starting a deletion request by accident.
            </Text>
            <TextInput
          ...
          <View style={styles.noteCard}>
            ...
            <Text style={styles.noteCopy}>
              Use this flow when you want Drapeon to process account closure and deletion. It is not
              an instant wipe while active orders, payouts, support cases, or legal retention
              obligations may still be open.
            </Text>
          </View>
          ...
          >
            {submitting ? (
              ...
              <Text style={[styles.actionBtnText, !canSubmit && styles.actionBtnTextDisabled]}>
                {providerReauth
                  ? `Continue with ${providerReauth === 'apple' ? 'Apple' : 'Google'}`
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              void openExternalUrl(
                `mailto:${CONTACTS.privacy}?subject=${encodeURIComponent('Drapeon tailor account deletion request')}`,
