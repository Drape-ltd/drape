...
export function StatusChip({ domain = 'generic', fallback = 'Not set', label, status, tone, ...props }: StatusChipProps): React.JSX.Element {
  const display = resolveStatusDisplay(status, { domain, fallback })
  return (
    <Badge tone={tone ?? display.tone} {...props}>
      <span className="truncate">{label ?? display.label}</span>
    </Badge>
  )
}
