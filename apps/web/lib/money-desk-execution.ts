...
import { buildRefundOrderPaymentsRequest } from '@drape/shared'
...
import type { OpsSession } from './ops-auth'
...
function actorLabel(session: OpsSession) {
...
async function runAdapter(client: ServiceRoleClient, session: OpsSession, moneyRequest: {
  id: string
  ...
  if (moneyRequest.action_type === 'CUSTOMER_REFUND') {
    if (moneyRequest.target_type === 'CONSULTATION_BOOKING') {
      ...
      return error ? { ok: false, error: `Refund completed but consultation finalization failed: ${error.message}` } : refund
    }
    ...
    if (moneyRequest.target_type === 'ORDER_MATERIAL_ADVANCE') {
      ...
      if (error || !moneyRequest.order_id || !moneyRequest.amount) return { ok: false, error: error?.message ?? 'The unused fabric value refund could not be prepared.' }
  ...
  if (moneyRequest.action_type === 'PAYOUT_DESTINATION_CHANGE') {
    if (moneyRequest.target_type === 'PAYOUT_CHANGE_REQUEST') {
      ...
      if (error || !change?.id || change.status !== 'PENDING') return { ok: false, error: error?.message ?? 'The payout destination request is no longer pending review.' }
    ...
    if (error || typeof correctionData.correctionId !== 'string') return { ok: false, error: error?.message ?? 'The reviewed payout destination correction could not be applied.' }
