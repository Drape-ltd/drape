...
const workspaceRoutes = [
  '/account/dashboard', '/account/explore', '/account/saved', '/account/brief',
  '/account/orders', '/account/messages', '/account/measurements', '/account/shop',
  '/account/items', '/account/checkout', '/account/work', '/account/earnings',
  '/account/payout', '/account/profile', '/account/notifications', '/account/settings', '/account/support',
  '/account/tailors', '/account/call-join',
]
...
function surfaceForPath(pathname: string): AccountSurface {
  ...
  const known: Partial<Record<string, AccountSurface>> = {
    dashboard: 'work', explore: 'explore', saved: 'saved', orders: 'orders',
    messages: 'messages', measurements: 'measurements', shop: 'shop', work: 'work',
    earnings: 'earnings', payout: 'payout', profile: 'profile', notifications: 'notifications', settings: 'settings',
    support: 'support',
  }
...
async function loadIdentity(session: Session): Promise<AccountRouteIdentity> {
  ...
  const storedRole = userResult.data?.role === 'CUSTOMER' || userResult.data?.role === 'TAILOR'
    ? userResult.data.role
    : null
  const requestedRole = sessionRole(session) ?? storedRole
  const role: AccountRouteIdentity['role'] = requestedRole === 'CUSTOMER' && customer
    ? 'CUSTOMER'
  ...
  const activeProfileName = role === 'TAILOR'
...
function StandaloneAccountRouteRuntime({ surface, children }: { surface: AccountSurface; children: (context: AccountRuntimeContextValue) => ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function acceptSession(session: Session | null) {
      ...
      try {
        ...
        if (active) setState({ status: 'error', message: error instanceof Error ? error.message : 'Your account could not load.' })
      }
    }
  ...
  if (state.status === 'loading') return <main className="grid min-h-screen place-items-center bg-ui-canvas"><p className="text-sm font-semibold text-ink/60">Loading your account…</p></main>
...
export function AccountRouteRuntime({ surface, children }: { surface: AccountSurface; children: (context: AccountRuntimeContextValue) => ReactNode }) {
  const inherited = useContext(AccountRuntimeContext)
  return inherited ? children(inherited) : <StandaloneAccountRouteRuntime surface={surface}>{children}</StandaloneAccountRouteRuntime>
}

export function PersistentAccountRuntime({ children }: { children: ReactNode }) {
  const pathname = usePathname() || '/account'
  const managed = workspaceRoutes.some((route) => pathname === route || pathname.startsWith(`${route}/`))
  if (!managed) return children
  return (
    <AccountRouteRuntime surface={surfaceForPath(pathname)}>
      {(context) => <AccountRuntimeContext.Provider value={context}>{children}</AccountRuntimeContext.Provider>}
    </AccountRouteRuntime>
  )
}
